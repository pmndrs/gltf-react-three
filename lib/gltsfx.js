...
type GLTFActions = Record<ActionName, THREE.AnimationAction>;\n`
  ...
  nodes: {
    ${meshes.map(({ name, type }) => `${createObjectKey(name)}: THREE.${type}`).join(',')}
    ${bones.map(({ name, type }) => `${createObjectKey(name)}: THREE.${type}`).join(',')}
  }
  materials: {
    ${materials.map(({ name, type }) => `${createObjectKey(name)}: THREE.${type}`).join(',')}
  }
}\n${animationTypes}`
  }
  ...
  function print(objects, gltf, obj, parent) {
    ...
    if (
      obj.name.length &&
      (obj.morphTargetDictionary || (hasAnimations && gltf.animations.find((clip) => clip.name.includes(obj.name))))
    )
    ...
    // Remove empty groups
    if ((type === 'group' || type === 'scene') && (result === oldResult || obj.children.length === 0)) {
      obj.__removed = true
      return children
    }
...
export default function Model(props${types ? ": JSX.IntrinsicElements['group']" : ''}) {
