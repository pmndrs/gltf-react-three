...
export function parse(fileName, arrayBuffer, exportOptions) {
  let options = {};
...
type GLTFActions = Record<ActionName, THREE.AnimationAction>;\n`;
  ...
  return new Promise((resolve, reject) => {
    Object.keys(exportOptions).forEach(
      (key) => (options[key] = exportOptions[key])
    );
...
export default function Model(props${
  ...
  const { nodes, materials${
    hasAnimations ? ", animations" : ""
        ...
        })${options.types ? " as GLTFResult" : ""}${printAnimations(
          gltf,
          animations,
          options
        )}
